(() => {
    ...
    (() => {
        ...
        window.addEventListener("load", (() => {
            const aboutItemsCounter = () => {
                const itemsNum = document.querySelectorAll(".about__item");
                itemsNum.forEach(((item, index) => {
                    item.querySelector(".about__num").textContent = index + 1;
                }));
            };
            aboutItemsCounter();
        }));
        const sizesFn = (width, height) => {
            const images = document.querySelectorAll("img,svg");
            images.forEach((img => {
                console.dir(img);
                width = img.naturalWidth;
                height = img.naturalHeight;
                return img.setAttribute("width", `${width}`), img.setAttribute("height", `${height}`);
            }));
        };
        sizesFn();
        isWebp();
